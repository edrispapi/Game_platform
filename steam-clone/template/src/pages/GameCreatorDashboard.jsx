...
const TabButton = ({ active, icon: Icon, label, onClick }) => (
  <button
    ...
    className={`flex items-center space-x-2 px-4 py-2 rounded-lg border transition-colors ${
      active ? 'bg-primary text-white border-primary' : 'bg-dark-800 border-gray-800 hover:border-primary/60'
    }`}
  >
    <Icon />
    <span>{label}</span>
  ...
  <div className="grid md:grid-cols-2 gap-6">
    ...
    <div className="bg-dark-800 border border-gray-800 rounded-xl p-6 space-y-4">
      ...
      <p className="text-sm text-gray-400">
        Monitor player retention, monetization funnels, and build health directly inside the dashboard. Configure automated alerts to stay ahead of regressions.
      </p>
...
const fallbackDashboardData = {
  ...
  share_links: [
    { label: 'Stakeholder Build', access: 'Password protected', url: 'https://vibecoding.ai/builds/neon/qa' },
    { label: 'Playtester Cohort', access: 'Invite only', url: 'https://vibecoding.ai/builds/neon/playtest' },
    { label: 'Internal Sandbox', access: 'Studio only', url: 'https://vibecoding.ai/builds/neon/internal' },
    { label: 'Marketing Demo', access: 'Public link', url: 'https://vibecoding.ai/builds/neon/press' },
  ],
};

const GameCreatorDashboard = () => {
  const [activeTab, setActiveTab] = useState('editor');
  const [script, setScript] = useState(fallbackDashboardData.script);
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        const response = await fetch('/api/v1/frontend/dashboard');
        if (!response.ok) {
          throw new Error('Unable to load creator workspace data.');
        }
        ...
        setDashboardData({
          build_status: payload.build_status || fallbackDashboardData.build_status,
          assets: Array.isArray(payload.assets) && payload.assets.length ? payload.assets : fallbackDashboardData.assets,
          share_links:
            Array.isArray(payload.share_links) && payload.share_links.length
              ? payload.share_links
              : fallbackDashboardData.share_links,
        });
        setScript(payload.script || fallbackDashboardData.script);
        setError(null);
      } catch (err) {
        if (!isMounted) return;
        setDashboardData(fallbackDashboardData);
        setScript(fallbackDashboardData.script);
        setError(err.message || 'Something went wrong while loading your workspace.');
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  const buildStatus = useMemo(() => dashboardData.build_status, [dashboardData.build_status]);
  const assets = useMemo(() => dashboardData.assets, [dashboardData.assets]);
  const shareLinks = useMemo(() => dashboardData.share_links, [dashboardData.share_links]);
    ...
    <div className="max-w-6xl mx-auto px-6 py-16 space-y-10">
      <header className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500">{isLoading ? 'Syncing data…' : 'Up to date'}</span>
        </div>
        <p className="text-gray-400 max-w-2xl">
          Command your AI-assisted pipeline with modular tooling for scripts, assets, builds, and distribution—all in one
          polished workspace.
        </p>
      ...
      <section className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-8">
          {activeTab === 'editor' && <EditorPane script={script} onScriptChange={setScript} />}
        ...
        <aside className="space-y-6">
          ...
          <div className="bg-dark-800 border border-gray-800 rounded-xl p-6 space-y-4">
            ...
            <p className="text-sm text-gray-400 leading-relaxed">
              Stay aligned with QA, marketing, and live ops milestones. Insights sync automatically with your connected
              services through the gateway.
            </p>
            <button className="inline-flex items-center space-x-2 px-4 py-2 border border-primary/60 hover:border-secondary transition-colors rounded-lg font-semibold">
              View go-to-market checklist
            </button>
          </div>
        </aside>
      </section>

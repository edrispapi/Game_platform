...
def update_review(
    ...
    """Update review"""
    review = crud.update_review(db=db, review_id=review_id, review_update=review_update)
    if not review:
        raise HTTPException(status_code=404, detail="Review not found")
    return review
...
def get_review_comments(
    ...
    return crud.get_review_comments(db=db, review_id=review_id, skip=skip, limit=limit)
...
def vote_review(
    ...
    """Vote on a review"""
    return crud.vote_review(db=db, review_id=review_id, user_id=user_id, is_helpful=is_helpful)